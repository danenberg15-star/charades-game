...
export default function FamilyAliasApp() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (step === 4) {
        ...
        else updateRoom({ step: 6, poolIndices: { ...roomData.poolIndices, JUNIOR: (roomData.poolIndices?.JUNIOR || 0) + 1 } });
      }
    }, 1000);
  ...
  const handleScoreAction = (targetName: string, points: number = 1) => {
    ...
    if (roomData.currentPhase === 'A') {
      ...
      const updatedDeck = [...(roomData.gameDeck || []), (roomData.shuffledPools?.JUNIOR || [])[roomData.poolIndices?.JUNIOR % (roomData.shuffledPools?.JUNIOR?.length || 1)]];
      if (updatedDeck.length >= roomData.players.length * 5) {
        const finalPool = shuffleArray(updatedDeck);
        updateRoom({ totalScores: newScores, poolIndices: { JUNIOR: 0 }, shuffledPools: { JUNIOR: finalPool }, currentPhase: 'B', gameDeck: updatedDeck, step: 6 });
      ...
      if (newIndices.JUNIOR >= (roomData.shuffledPools?.JUNIOR?.length || 0)) {
        if (roomData.currentPhase === 'B') updateRoom({ totalScores: newScores, currentPhase: 'C', poolIndices: { JUNIOR: 0 }, step: 6 });
        else {

...
export default function SetupStep(props: SetupStepProps) {
  ...
  return (
    ...
    >
      ...
      {/* Header - צבע קוד החדר שונה לטורקיז */}
      <div style={{...styles.setupHeader, width: '100%', boxSizing: 'border-box', padding: '0 5px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px'}}>
        <div style={{ color: 'white', fontSize: '1rem', display: 'flex', alignItems: 'center' }}>
          קוד: <span style={{ color: '#00f2ff', fontWeight: '900', fontSize: '1.6rem', marginRight: '5px' }}>{props.roomId}</span>
        </div>
      ...
      {/* Players Grid - מסגרות וצבעי שמות שונו לטורקיז */}
      <div style={{ 
        ...styles.setupGrid, 
        ...
        {Array.from({ length: props.gameMode === "team" ? props.numTeams : 1 }).map((_, tIdx) => {
          ...
          return (
            <div key={tIdx} ref={el => { teamRefs.current[tIdx] = el; }} style={{ 
              ...
              boxSizing: 'border-box',
              touchAction: 'none',
              ...(hoveredTeam === tIdx ? { borderColor: '#00f2ff', backgroundColor: 'rgba(0, 242, 255, 0.1)' } : {}) 
            }}>
              <div style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '4px', textAlign: 'center', fontSize: '0.75rem', color: '#00f2ff', fontWeight: 'bold' }}>
        ...
        {props.gameMode === "team" && props.numTeams < 4 && !hasEmptyTeam && (
          <button onClick={handleAddTeam} style={{ ...styles.teamBox, borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center', minHeight: 0, width: '100%', boxSizing: 'border-box' }}>
            <span style={{ fontSize: '1.5rem', color: '#00f2ff' }}>+</span>
          </button>
        )}
      </div>
      ...
      {draggedPlayer && (
        <div ref={ghostRef} style={{ position: 'fixed', zIndex: 9999, pointerEvents: 'none', backgroundColor: '#00f2ff', padding: '10px', borderRadius: '12px', color: '#05081c', fontWeight: 'bold', width: '100px', textAlign: 'center' }}>
          {draggedPlayer.name}
        </div>
      )}
    </div>

...
export default function GameStep({ roomData, userId, targets, updateRoom, handleAction, onExit }: any) {
  ...
  const wordData = useMemo(() => {
    ...
    return pool[idxs[poolKey] % (pool.length || 1)] || { word: "טוען...", en: "" };
  ...
  const s: any = {
    ...
    card: { width: '100%', maxWidth: '320px', height: '100%', maxHeight: '300px', backgroundColor: '#1a1d2e', borderRadius: '35px', boxShadow: '0 20px 50px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' },
    hebL: { fontSize: 'clamp(1.8rem, 8vw, 2.5rem)', fontWeight: '900', textAlign: 'center', wordBreak: 'break-word' }, 
    ...
    pauseBox: { width: '100%', height: '100%', backgroundColor: '#1a1d2e', borderRadius: '35px', padding: '20px', display: 'flex', flexDirection: 'column', boxSizing: 'border-box' },

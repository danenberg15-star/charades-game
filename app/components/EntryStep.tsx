...
const localStyles: { [key: string]: CSSProperties } = {
  ...
  addBtn: { backgroundColor: '#00f2ff', color: '#05081c', border: 'none', borderRadius: '10px', padding: '10px', fontWeight: '900', cursor: 'pointer' },
  ...
  return (
    <div style={localStyles.flexLayout}>
      ...
      <div style={localStyles.formSection}>
        ...
        <div style={localStyles.customBox}><label style={localStyles.label}>הוספת שמות אישיים לחבילה:</label><input placeholder="שם בעברית (למשל: דודה שרה)" value={newHeb} onChange={e => setNewHeb(e.target.value)} style={{...localStyles.entryInput, height: '2.8em', fontSize: '0.9rem'}} /><select value={newCat} onChange={e => setNewCat(e.target.value)} style={{...localStyles.entryInput, height: '2.8em', fontSize: '0.9rem', marginTop: '5px'}}>{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select><button onClick={() => { if(newHeb.trim()){setCustomWords([...customWords, { word: newHeb.trim(), en: "", category: newCat }]); setNewHeb("");} }} style={localStyles.addBtn}>+ הוסף שם</button><div style={localStyles.wordList}>{customWords.map((w, i) => (<span key={i} style={localStyles.wordTag}>{w.word}</span>))}</div></div>
        <div style={{...localStyles.inputGroup, marginTop: '10px'}}><input type="text" value={inputCode} onChange={(e) => setInputCode(e.target.value.toUpperCase())} placeholder="יש לך קוד חדר?" style={{...localStyles.entryInput, borderStyle: 'dashed'}} />
        <button onClick={() => startAction('join')} style={localStyles.primaryButton}>הצטרפות</button>
        {initialCode && <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.8rem', textAlign: 'center', marginTop: '5px' }}>כניסה אוטומטית לחדר: {inputCode}</p>}</div>
      </div>
